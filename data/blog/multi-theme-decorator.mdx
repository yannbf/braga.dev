...
![](/public/static/images/multi-theme-decorator/theme-errors.png)

That makes sense. As the component renders in isolation, it needs a ThemeProvider with the themes for the `useTheme` hook to work. Normally in an application, that is available higher in the tree. In Storybook, however, that should be provided via a decorator.

## Creating the theme decorator

The first step is to actually add support for themes in Storybook. Let's start simple and create a `withTheme` decorator which wraps the story with a hardcoded `dark` theme, and also overrides the styles of `body` to use the app background color:
...
![](/public/static/images/multi-theme-decorator/with-dark-theme.png)
...
![](/public/static/images/multi-theme-decorator/theme-picker.gif)
...
2. You can change the `defaultTheme` in the decorator logic by leveraging the [isChromatic](https://www.chromatic.com/docs/ischromatic#check-for-chromatic) helper, which will return true only when Chromatic is capturing a snapshot. This way every component you have will have its snapshot taken in all themes:
