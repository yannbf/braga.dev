...
We can modify the `withRTL` decorator code to extract the `parameters` property from the story context, and make the decorator only apply the RTL mode if the story contains `parameters.rtl: true`:

```js:.storybook/preview.js {2,4}
const withRTL = (StoryFn, context) => {
...
const withRTL = (StoryFn, context) => {
  ...
  useEffect(() => {
   // Set RTL only if passed as a parameter or toggled via toolbar
    const direction = (parameters.rtl || globals.textDirection === 'rtl') ? 'rtl' : 'ltr'
